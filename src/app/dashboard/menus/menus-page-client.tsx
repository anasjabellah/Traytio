...
const CreateMenuDialog = dynamic(() => import('@/features/menus/components/create-menu-dialog').then((m) => m.CreateMenuDialog), { loading: () => null });
...
import type { Menu, MenuCategory, PaginatedMenus } from '@/features/menus/types';
...
export function MenusPageClient({ initialData }: MenusPageClientProps) {
  return (
    <PageGuard module="menus" action="read">
      <MenusPageContent initialData={initialData} />
    </PageGuard>
  )
}

function MenusPageContent({ initialData }: { initialData?: PaginatedMenus | null }) {
