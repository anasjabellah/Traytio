...
function InvoicesPageContent({ initialData }: { initialData?: PaginatedResult<InvoiceWithCommande> | null }) {
  ...
  const [invoices, setInvoices] = useState<InvoiceWithCommande[]>(initialData?.data ?? [])
  ...
  const shouldSkipInitialFetch = initialData && page === 1 && limit === 10 && !typeFilter && !search
  ...
  }, [refetch, shouldSkipInitialFetch])
