...
const DeleteConfirmDialog = dynamic(() => import("@/components/ui/delete-confirm-dialog").then((m) => m.DeleteConfirmDialog), { loading: () => null })
...
interface TeamPageClientProps {
  initialData?: {
    members: TeamMember[];
    invitations: import("@/features/team/types").TeamInvitation[];
    stats: TeamStats;
    pagination: import("@/features/team/types").TeamPagination;
  } | null;
}

export default function TeamPageClient({ initialData }: TeamPageClientProps) {
  const { can, role: currentRole } = useRole()
  const [page, setPage] = useState(1)
  const [limit, setLimit] = useState(20)
  const { members, invitations, pagination, isLoading, error, stats, totalKpi, activeKpi, inviteKpi, adminKpi, refresh } = useTeam(page, limit, initialData)
