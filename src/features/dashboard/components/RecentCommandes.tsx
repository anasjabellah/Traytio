    ...
    >
      <div className="flex flex-col gap-2 flex-1 min-w-0">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium tabular-nums">{c.number}</span>
          <ChevronRight className="size-4 text-muted-foreground shrink-0 ml-2" />
        </div>
        <div className="flex items-center gap-2 text-sm">
          <div className="size-6 rounded-full bg-gradient-to-br from-foreground/10 to-foreground/5 flex items-center justify-center text-[10px] font-medium shrink-0">
  ...
  return (
    <div className="rounded-2xl border border-border bg-card shadow-soft overflow-hidden">
          ...
          {commandes.map((c, i) => (
            <motion.div key={c.id}
              ...
              <div className="col-span-3 flex items-center gap-2 text-sm truncate">
                <div className="size-7 min-h-[44px] min-w-[44px] md:min-h-0 md:min-w-0 rounded-full bg-gradient-to-br from-foreground/10 to-foreground/5 flex items-center justify-center text-[10px] font-medium shrink-0">
                  {c.clientName.split(" ").map((n) => n[0]).join("").slice(0, 2)}
                </div>
      ...
      <div className="sm:hidden divide-y divide-border">
        ...
        {commandes.map((c, i) => (
          <CommandeMobileRow key={c.id} c={c} i={i} />
        ))}
      </div>
    </div>
  );
